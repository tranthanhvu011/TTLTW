...
<%
    ...
    List<Integer> listIdSelected = (List<Integer>) session.getAttribute("listProductImport");
    if (listIdSelected == null) listIdSelected = new ArrayList<>();
    List<NhapHang> nhapHangList = (List<NhapHang>) request.getAttribute("nhaphang");
    if (nhapHangList == null) listIdSelected = new ArrayList<>();
    ...
    <div class="main-content">
        ...
        <div class="single-table"
            ...
            <div class="table-responsive">
                <!-- Table -->
                <form action="${pageContext.request.contextPath}/admin/importSuccess" method="post">
                    <table id="table_inventory" class="table text-center">
                        ...
                        <tr class="text-white">
                            ...
                            <th scope="col">Giá nhập 1 sản phẩm</th>
                            <th scope="col">Giá nhập toàn bộ</th>
                            <th scope="col">Chi nhánh nhập</th>
                            <th scope="col">Kho nhập</th>
                            <th scope="col">Hành động</th>
                        </tr>
                        </thead>
                        <tbody>
                        <% for (Integer idProduct : listIdSelected) { %>
                        <tr id="log-<%=idProduct%>">
                            <td><%=idProduct%></td>
                            <td><%=productVariantDAO.getProductNameByVariantId(idProduct)%></td>
                            <td>
                            </td>
                            <% ProductVariant productVariant = ProductVariantDAO.getColorAndCapacityProductVariantByID(idProduct); %>
                            <td><input type="number" name="quantity_<%=idProduct%>" placeholder="Nhập số lượng" value="0" min="1"></td>
                            <td><input type="number" name="priceOne_<%=idProduct%>" placeholder="Nhập giá" min="0"></td>
                            ...
                            <td>
                                <% List<ProductInventory> listInventory = productInventoryDAO.getProductInventoriesByVariantAndQuantity(idProduct); %>
                                <select style="width: 70%;" name="warehouse_<%=idProduct%>" required>
                                    <option value="">Chọn kho hàng</option>
                                    <% if (listInventory != null && !listInventory.isEmpty()) { %>
                                    <% for (ProductInventory kho : listInventory) { %>
                                    <option value="<%= kho.getId() %>"><%= inventoryDAO.getInventoryByID(kho.getIdKho()).getNameKho() %></option>
                                    <% } %>
                                    <% } else { %>
                                    <option value="" disabled>Không có kho hàng</option>
                                    <% } %>
                                </select>
                                <% if (listInventory == null || listInventory.isEmpty()) { %>
                                <p style="color: red;">Hiện tại sản phẩm này kho hàng đã hết, vui lòng chọn sản phẩm khác.</p>
                                <% } %>
                            </td>

                            <td><button type="submit" class="delete-product" name="action" value="delete" data-id="<%= idProduct %>">Delete</button></td>
                        </tr>
                        <% } %>
                        </tbody>
                    </table>
...
<script>
    $(document).ready(function() {
        $(document).on('click', '.delete-product', function() {
            ...
            $('#log-' + productId).remove();
            $.ajax({
                url: '/admin/manage_getlistimport',
                type: 'POST',
